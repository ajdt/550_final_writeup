	...
	\Procedure{Puddle}{$\EE_i$, $\PP$}
		...
		\Switch{$i$.state}
			\Case{START}
				...
				\If{status and not onLRTangent(i)} % insert snippet to move onto line l/r
...
\Procedure{leaderPattern()}{$\EE_i, \PP$}
	\If{$i$ is the bottom-most robot on $m$}
